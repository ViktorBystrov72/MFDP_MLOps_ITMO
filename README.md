...
#### 2.3 Ручной матчинг — аналитик (косвенный)
...
#### 2.4 Алгоритмический матчинг — правила (косвенный)
...
| **Цена (ориентир)** | от 46–63 тыс. ₽/мес | ~60 тыс. ₽/мес (регион) | ~150 тыс. ₽/мес (ФОТ) | Минимальная | Размещение + исследования | **≤ 50 тыс. ₽/мес** за модуль |
| **География** | 40 регионов | 15+ регионов | Зависит от аналитика | Любой ЖК в данных | Федерально (объявления) | Пилот: 1 ЖК → масштаб |
| **Глубина истории** | с 2016 г. | Ежедневно + Госисточники | По выгрузкам | По данным БД | По объявлениям | **≥ 3 мес.** на пилоте |
...
| **API** | Да (от 66 тыс. ₽/мес) | Да | Нет | Batch-скрипт | Ограниченно | **Да** (batch на MVP) |
...
#### Откуда взялся бенчмарк Precision ≥ 0.85
...
4. **Выгрузки и API** — без интеграции модуль не встроится в контур Neolithic.
...
- **Модуль к существующему договору Neolithic** — ниже порог входа, чем подписка на bnMAP/Объектив (46–63 тыс. ₽/мес).
- **Прозрачные метрики качества** (Coverage, Precision@match, Accuracy@3) — конкуренты этого не публикуют.
...
| Высокая стоимость полной платформы для малого девелопера | Продаём **модуль**, а не всю аналитику рынка |
...
- **Тарификация по объёму данных** (как Объектив) — справедливо для малых девелоперов с 1–3 ЖК.
...
- **Еженедельный markdown/CSV-отчёт** под сценарий пилота, а не перегруженный BI.

#### Ключевые характеристики для конкурентоспособности

1. Поквартирная связка экспозиция ↔ сделка с **Coverage ≥ 75%**.
2. **Precision ≥ 0.85** — чтобы не искажать данные застройщика.
3. Интеграция через **batch/API** в контур Neolithic.
4. Стоимость batch-прогона **≤ 500 ₽ / ЖК / неделя**.

### 5. Определение стандартов (бенчмарки Neolithic)

#### Минимальные требования к функциональности

- Матчинг квартир: экспозиция → сделка на пилотном ЖК.
- Нормализация адресов, блокирующие ключи (ЖК + корпус + площадь).
- Baseline (правила + fuzzy) + ML-классификатор пар.
- Batch-пайплайн, Streamlit-дашборд, очередь ручной проверки.
- Выгрузка CSV/Parquet + markdown-отчёт.
...
- Модульная цена vs полная платформа за 46–63 тыс. ₽/мес.
...
| Этап | Модель |
|------|--------|
| Пилот (6 недель) | Бесплатно / включено в договор Neolithic |
| После валидации | Доп. модуль: **~30–50 тыс. ₽/мес** за 1–3 ЖК (ниже bnMAP, сопоставимо с региональным тарифом Объектив) |
| Масштаб | Тарификация по числу ЖК и регионов + API |

#### Минимальные показатели (бенчмарки)

| Метрика | Benchmark (рынок / конкуренты) | Требование Neolithic |
|---------|-------------------------------|----------------------|
| География | 15–40 регионов у лидеров | Пилот: 1 ЖК; v2: по дорожной карте Neolithic |
| Глубина истории | с 2016 г. (bnMAP) | ≥ 3 мес. на пилоте |
| Поквартирная детализация | Есть у bnMAP и Объектив | Обязательно |
| API / выгрузки | Есть у bnMAP и Объектив | Batch + выгрузки на MVP |
| Coverage (жизненный цикл) | Не раскрывается у конкурентов | **≥ 75%** |
| Precision@match | ~0.95 (ручной) / ~0.65–0.70 (правила) / не раскрывается (bnMAP, Объектив) | **≥ 0.85** |
| Recall@match | Не раскрывается | **≥ 0.70** |
| Себестоимость прогона | Не раскрывается у конкурентов | **≤ 500 ₽ / ЖК / неделя** |
| Стоимость для клиента | 46–63 тыс. ₽/мес (полная платформа) | **≤ 50 тыс. ₽/мес** (модуль) |
...
- **Ориентиры для метрик** зафиксированы: Coverage, Precision, Recall, себестоимость — с конкретными порогами из раздела прототипа и подтверждением рыночными данными.
